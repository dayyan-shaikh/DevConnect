...
const Developers = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfiles = async () => {
      try {
        const response = await axios.get("http://localhost:5000/profile/profiles");
        if (Array.isArray(response.data.profiles)) {
          setProfiles(response.data.profiles);
        } else {
          console.error("Expected an array under response.data.profiles, got:", response.data);
        }
      } catch (error) {
    ...
    <div>
      ...
      <div className="bg-gray-700 py-16 flex flex-col items-center">
        <h2 className="text-4xl text-white">
          CONNECT WITH <span className="text-4xl font-bold text-white">DEVELOPERS</span>
          <br />
